...
CREATE TABLE financial_category_accounts(category_id TEXT,company_id TEXT,codigo_reduzido TEXT,PRIMARY KEY(category_id,company_id));
...
(async()=>{
  ...
  db.exec(`INSERT INTO companies VALUES ('same-office','one');
    INSERT INTO dominio_chart_of_accounts VALUES ('loan-a','a','501','2.1.1','Empréstimos','analitica','','');
    INSERT INTO dominio_chart_of_accounts VALUES ('loan-other','same-office','702','2.1.1','Empréstimos','analitica','','');
    INSERT INTO dominio_chart_of_accounts VALUES ('synthetic','a','500','2.1','Passivo','sintetica','','');`);
  ...
  assert.equal(other.result.data.id,linked.result.data.id);
  assert.equal(other.result.data.codigo_reduzido,'702');
  const listA=await call('getCategories',{query:{company_id:'a'}});
  assert.equal(listA.result.data[0].codigo_reduzido,'501');
  assert.equal(listA.result.data[0].conta_nome,'Empréstimos');
  assert.equal((await call('getCategories',{query:{company_id:'b'}})).code,403);
  db.exec("INSERT INTO dominio_chart_of_accounts VALUES ('other-a','a','503','2.1.2','Outro empréstimo','analitica','','')");
  assert.equal((await call('createCategory',{body:{...body,codigo_reduzido:'503'}})).code,409);
  assert.equal(db.prepare('SELECT codigo_reduzido FROM financial_category_accounts WHERE company_id=?').get('a').codigo_reduzido,'501');
  db.exec('DELETE FROM dominio_chart_of_accounts');
